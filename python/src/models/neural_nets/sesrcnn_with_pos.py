...
class ConfigSESRCNNWithPos(BaseModelConfig):
    in_channels: int
    is_globally_skipped: bool
    lr_tm_channel: int


class SESRCNNWithPos(nn.Module):
    def __init__(self, config: ConfigSESRCNNWithPos):
        super().__init__()

        logger.info(f"Input SESRCNNWithPos config = {config.to_json_str()}")
    ...
    def forward(self, x: torch.Tensor, pos: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        x = torch.cat([x, pos], dim=1)  # concat along channel
